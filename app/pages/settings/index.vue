...
const { data: userData, refresh } =
  await useFetch<ProfileResponse>("/api/profile");
...
onChange(async () => {
  ...
  try {
    ...
    toast.add({
      ...
      description:
        "Het kan even duren voordat de wijziging overal zichtbaar is.",
    });
  ...
  <UForm>
    ...
    <UPageCard variant="subtle" class="mb-4">
      <UFormField
        ...
        name="name"
      >
        <UInput v-model="profile.name" disabled autocomplete="off" />
      </UFormField>
      <USeparator />
      <UFormField
        class="flex max-sm:flex-col justify-between items-start gap-4"
