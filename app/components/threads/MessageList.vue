...
const expectedThreadCount = computed(() =>
  threadCount.value > 0 ? threadCount.value : 30,
);
...
watch(
  ...
  () => {
    if (issues.length)
      threadCount.value = Math.min(30, Math.max(5, issues.length));
  },
);
...
defineShortcuts({
  arrowdown: async () => {
    const index = issues.findIndex(
      (thread) => thread.id === selectedIssue.value?.id,
    );

    if (index === -1) selectedIssue.value = issues[0] ?? null;
    else if (index < issues.length - 1) selectedIssue.value = issues[index + 1];
  },
  arrowup: async () => {
    const index = issues.findIndex(
      (thread) => thread.id === selectedIssue.value?.id,
...
<template>
  ...
  <template v-else>
    ...
    <template v-else>
      <p v-for="issue in issues" :key="issue.id">Hello issue {{ issue.id }}</p>
    </template>
  </template>
</template>
