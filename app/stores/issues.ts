...
export const useIssueStore = defineStore("issueStore", {
  ...
  getters: {
    activeIssue(state) {
      return state.activeIssueId !== null
        ? (state.issues.find((ticket) => ticket.id === state.activeIssueId) ??
            null)
        : null;
    },
  },
  actions: {
    async fetch() {
      ...
      this.unreadIssues = issues
        .filter((ticket) => !ticket.read)
    ...
    setActiveIssue(issue: number | Issue | null) {
      if (issue === null) this.activeIssueId = null;
      else if (typeof issue === "number") this.activeIssueId = issue;
      else this.activeIssueId = issue.id;
    },
  },
});
