...
export function useAuth() {
  ...
  const { data: sessionData, status } = useAsyncData<BetterAuthSessionResponse>(
    "urn:local:session",
    async (): Promise<BetterAuthSessionResponse> => {
  ...
  const user = computed<User | null>(
    () => sessionData.value?.data.user ?? null,
  );
  const session = computed<Session | null>(
    () => sessionData.value?.data.session ?? null,
  );
  ...
  watch(status, (oldStatus, newStatus) => {
    if (newStatus === "pending") isLoading.value = true;
  ...
  const signIn = async () => {
    ...
    try {
      ...
      error.value =
        (err instanceof Error ? err.message : null) ?? "Sign in failed";
    } finally {
      isLoading.value = false;
    }
  };
