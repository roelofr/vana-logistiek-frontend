...
export default defineNuxtPlugin(() => {
  ...
  const baseOrigin = computed(
    () => new URL(baseUrl.value, "https://example.com").origin,
  );
  ...
  const $api = $fetch.create({
    ...
    onRequest({ request, options }) {
      ...
      options.headers.set(
        "Accept",
        "application/json, image/*, text/*;q=0.9, */*;q=0.8",
      );
    },
    onResponse(res) {
    ...
    onResponseError(response) {
      if (toastRef.value == null && response.response.status === 401) {
        toastRef.value = toast.add({
          ...
          description:
            "Je sessie is verlopen, herlaad de pagina om opnieuw in te loggen.",
          duration: -1,
          actions: [
            {
  ...
  const resolve = (path: string): string => {
    ...
    if (pathAsUrl.origin !== baseOrigin.value)
      throw new Error("Path must not specify an origin.");

    // Trim the origin and the first slash and join it together with the base path
    const pathOnly = pathAsUrl
      .toString()
      .substring(pathAsUrl.origin.length + 1);
